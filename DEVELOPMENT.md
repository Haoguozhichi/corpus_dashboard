...
|------|------|
| 实验统计 | 实验总数统计 |
| 实验列表 | 表格展示所有实验（名称、负责人、日期、实验组数），支持编辑和删除 |
| 搜索筛选 | 按名称搜索、负责人筛选、日期范围筛选 |
| 创建实验 | 弹出表单，填写名称/描述/负责人/日期 |
| 行点击 | 点击实验行进入该实验的仪表盘 |

### 4.2 仪表盘 `/experiment/:id`
...
| 统计卡片 | 实验组数、最高正确率、最佳组 |
| 管理实验组 | 表格统一编辑所有实验组的名称/模型/变量，支持添加/删除实验组和变量列 |
| 实验组对比表 | 每个实验组一行，变量展开为列，支持排序筛选 |
| 子分组列 | 默认折叠，点击展开显示各子分组准确率 |
| 一键导入 | JSON 文件上传，自动创建实验组和结果 |
| 导出 | 将实验数据导出为 JSON 文件下载 |
| AI 报告 | 点击生成/查看 LLM 自动生成的实验报告 |
| 对比分析 | 勾选多个实验组，点击对比进入对比分析页 |
| 行点击 | 点击实验组行进入该组的详情页 |

### 4.3 评测详情页 `/experiment/:id/group/:gid`

**作用**：展示单个实验组的所有评测结果。

**表格列**：
```
# | 子分组 | 题目 | 标准答案 | 模型回答 | 自定义字段... | 结果 | 耗时 | Token | 原因 | 标注 | Think
```
...
| 管理评测结果 | 可编辑表格：批量编辑/添加行/添加自定义列/删除/JSON批量上传 |

### 4.4 Agent 详情页

与评测详情页类似，额外显示轨迹弹窗和 AI 轨迹诊断。

### 4.5 对比分析页 `/experiment/:id/compare`
...
1. 首页点击「创建实验」→ 填写名称/日期/负责人
2. 进入实验仪表盘 → 点击「一键导入」→ 上传 JSON 文件
3. 导入完成后仪表盘显示所有实验组和指标

### 5.3 管理实验组

点击「管理实验组」→ 表格中编辑所有实验组的名称/模型/变量，支持添加/删除行和变量列

### 5.4 使用 AI 分析

1. 顶部导航栏点击「LLM」→ 配置 API 地址、模型名称、API Key → 测试连接 → 保存
2. 进入实验组详情 → 勾选错误用例 → AI 分析下拉 → 选择诊断/聚类
