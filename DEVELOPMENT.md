...
│  ┌──────────┐ ┌───────────┐ ┌──────────┐ ┌─────────┐  │
│  │categories│ │experiments│ │ groups   │ │ tests   │  │
│  │  CRUD    │ │ CRUD+导入  │ │ CRUD     │ │ CRUD    │  │
│  └──────────┘ └───────────┘ └──────────┘ └─────────┘  │
│  ┌──────────┐ ┌───────────┐ ┌──────────────────────┐   │
│  │ metrics  │ │  results  │ │        llm           │   │
│  │ 训练指标  │ │ 评测结果   │ │ 错误诊断/标注/报告    │   │
│  └──────────┘ └───────────┘ └──────────────────────┘   │
└────────────────────────┬───────────────────────────────┘
                         │ file I/O
┌────────────────────────▼───────────────────────────────┐
│                server/data.json                         │
│   { categories: [{ experiments: [{ groups: [...] }] }]} │
└────────────────────────────────────────────────────────┘
```
...
Category (实验类别)
  │ 1:N
  ▼
Experiment (实验)  type = "evaluation"
  │ 1:N                    │ 1:N
  ▼                        ▼
ExperimentGroup (实验组)   TestCase (测试用例)
  │ 1:N
  ▼
EvaluationResult (评测结果)
  ├── question, expected_answer, model_response
  ├── is_correct, runtime_ms, token_count
  ├── reason, annotation
  ├── trajectory[] (Agent)
  │     └── think, thought, action, observation, tool
  └── 自定义字段 (difficulty, table_count, ...)
```
